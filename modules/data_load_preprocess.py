...
def data_loading_and_preprocessing():
    ...
    station_names = ['cdp', 'oas', 'obs', 'ojp', 'rme',
                     'sap', 'snb', 'sod', 'swa', 'wfj']
    ...
    for station_idx, station_name in enumerate(station_names):
        ...
        file_path_met = os.path.join('data','ESM-SnowMIP_all', f'{filname_met}.nc')
        file_path_obs = os.path.join('data','ESM-SnowMIP_all', f'{filename_obs}.nc')
        ...
        df_data = pd.concat([df_met_preprocessed,
                             df_obs_preprocessed,
...
def mod_preprocessing(dataset_mod):
    ...
    datetime_index = pd.date_range(start=start_datetime, periods=len(df_mod), freq='H')
...
def met_preprocessing(df_met, lag, lat_station, lng_station):
    ...
    names_met_agg = ['Psurf_avg', 'Qair_avg', 'Qair_dav', 'Rainf_avg',
